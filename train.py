...
def main(args):
    ...
    if use_cfg and args.class_conditioning:
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            ### Sample initialGaussian noise and time to perform interpolation ###
            ### Be consistent with the notation used by SiT, x1 is the clean latent and x0 is the initial Gaussian noise. The sampling direction is from t=0 -> t=1.
            x0 = torch.randn_like(x1)
            t = torch.rand(local_batch_size, device=device)
            ...
            loss = F.mse_loss(model_output, v)
            opt.zero_grad()
            loss.backward()
            opt.step()
            ...
            ### Sample EMA samples if needed ###
            ### Be aware that sampling will temporarily increase the VRAM usage, so you many experience CUDA OOM even when training is fine.
            if train_steps % args.sample_every == 0 and train_steps > 0 and args.sample:
                logger.info("Generating EMA samples...")
                sample_fn = sampler.sample_ode() # default to ode sampling
                if args.class_conditioning:
                    samples = sample_fn(zs, model_fn, **sample_model_kwargs)[-1]
                else:
                    samples = sample_fn(zs, model_fn)[-1]
                dist.barrier()
